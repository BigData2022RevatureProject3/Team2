...
object eCommerceWebsites{
  ...
  def main(args: Array[String]): Unit = {
    ...
    try{
      var products = spark.read.option("header","true").csv("data/ecommerceWebsites.csv")
      eCommWebsiteGenerator(products)
    }catch {
...
*/
  ...
  def eCommWebsiteGenerator(e:DataFrame):Unit = {
    var output = ArrayBuffer[String]()
    output.foreach(x =>{
      output = gen(x(0).toInt, output, e)
    })
    val out = Random.shuffle(output.toList).toArray.foreach(println)
    Thread.sleep(2000)
    eCommWebsiteGenerator(e)
  }

  def gen(m:Int, output:ArrayBuffer[String], products:DataFrame):ArrayBuffer[String] = {
    var quantity = 0
    val max = m

    val list = products.select("*").where(s"ecommerce_website_name = ''").collect()
    while(quantity != max) {
